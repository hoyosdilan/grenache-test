...
const ServiceOrderBook = require('./common/entity/OrderBook');
...
setInterval(() => {
  console.log('announce orderbook_service');
...
service.on('request', (rid, key, payload, handler) => {
  ...
  switch (payload.method) {
    ...
    case 'get_all_orders':
      try {
        const orders = orderbook.getAllOrderBook();
        handler.reply(null, orders);
      } catch (error) {
        handler.reply(error, { success: false });
      }
...
function handleGetOrder() {
  return serviceOrderBook.addOrder(order);
}
